...
impl Into<Vec<u8>> for Request {
    fn into(self) -> Vec<u8> {
        ...
        match self {
            Request::TcpConnect(passcode, address) => {
                buf.extend_from_slice(&passcode);
...
impl Streamable for Request {
        ...
        T: AsyncRead + Unpin + Send,
    {
        ...
        stream.read_exact(&mut passcode).await?;
        ...
        match request_type {
            Self::RTYP_TCP_CONNECT => Ok(Request::TcpConnect(passcode, address)),
            _ => Err(io::Error::new(
                io::ErrorKind::InvalidData,
...
mod tests {
    ...
    async fn test_domain_request() {
        ...
        let port = 80;
        let request = Request::TcpConnect(passcode, Address::Domain(domain.clone(), port));
        ...
        match parsed_request {
            Request::TcpConnect(parsed_passcode, Address::Domain(parsed_domain, parsed_port)) => {
                assert_eq!(passcode, parsed_passcode);
    ...
    async fn test_ipv4_request() {
        ...
        let request = Request::TcpConnect(passcode, Address::IPv4(addr));
        ...
        match parsed_request {
            Request::TcpConnect(parsed_passcode, Address::IPv4(parsed_addr)) => {
                assert_eq!(passcode, parsed_passcode);
    ...
    async fn test_ipv6_request() {
        ...
        let request = Request::TcpConnect(passcode, Address::IPv6(addr));
        ...
        match parsed_request {
            Request::TcpConnect(parsed_passcode, Address::IPv6(parsed_addr)) => {
                assert_eq!(passcode, parsed_passcode);
                assert_eq!(addr.ip(), parsed_addr.ip());
                assert_eq!(addr.port(), parsed_addr.port());
            }
    ...
    async fn test_special_chars_domain() {
        ...
        for domain in special_domains {
            let port = 443;
            let request = Request::TcpConnect(passcode, Address::Domain(domain.to_string(), port));
    ...
    async fn test_edge_case_ports() {
        ...
        for port in edge_ports {
            ...
            let request = Request::TcpConnect(passcode, Address::IPv4(ipv4_addr));
            ...
            let request = Request::TcpConnect(passcode, Address::IPv6(ipv6_addr));
    ...
    async fn test_partial_read() {
        let passcode = [0u8; 32];
        let request = Request::TcpConnect(passcode, Address::Domain("example.com".to_string(), 80));
        let bytes: Vec<u8> = request.into();
    ...
    async fn test_empty_domain() {
        let passcode = [0u8; 32];
        let request = Request::TcpConnect(passcode, Address::Domain("".to_string(), 80));
        let bytes: Vec<u8> = request.into();
        let mut cursor = Cursor::new(bytes);

        let parsed_request = Request::read(&mut cursor).await.unwrap();
        match parsed_request {
            Request::TcpConnect(parsed_passcode, Address::Domain(domain, port)) => {
                assert_eq!(passcode, parsed_passcode);
                assert_eq!(domain, "");
                assert_eq!(port, 80);
            }
    ...
    async fn test_request_roundtrip() {
        let passcode = [0u8; 32];
        let test_cases = vec![
            Request::TcpConnect(passcode, Address::Domain("example.com".to_string(), 80)),
            Request::TcpConnect(
                passcode,
...
mod advanced_tests {
    ...
    async fn test_large_domain_stress() {
        ...
        let domains = (1..100).map(|i| {
            ...
            Request::TcpConnect(passcode, Address::Domain(domain, 80))
        });
        ...
        for request in domains {
            ...
            match (request, parsed_request) {
                (
                    Request::TcpConnect(orig_passcode, Address::Domain(orig_domain, orig_port)),
                    Request::TcpConnect(
                        parsed_passcode,
                        Address::Domain(parsed_domain, parsed_port),
                    ),
                ) => {
                    assert_eq!(orig_passcode, parsed_passcode);
