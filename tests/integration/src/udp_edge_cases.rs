...
async fn test_udp_empty_datagram() -> io::Result<()> {
    ...
    let (response, from_addr) = udp_session.recv_from().await.ok_or_else(|| {
        io::Error::new(io::ErrorKind::UnexpectedEof, "udp_session channel closed")
    })?;
...
async fn test_udp_invalid_domain_name() {
    ...
    let udp_session = client.open_associate();
...
async fn test_udp_session_cleanup() -> io::Result<()> {
...

    {
        ...
        // Receive and discard the echo response to ensure the first session's data is fully processed
        let mut buf = [0u8; 1024];
        let (len, from) = echo_server.recv_from(&mut buf).await?;
        assert_eq!(&buf[..len], message.as_ref());
        echo_server.send_to(&buf[..len], from).await?;

        // Try to receive the response, but it may be dropped when session is dropped
        let _ = udp_session.recv_from().await;
